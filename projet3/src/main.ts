...
type SalesData = Record<string, MonthlyData[]>;
...
try {
  ...
  const updateCharts = (data: MonthlyData[]) => {
  ...
  const yearChange$ = fromEvent(yearSelect, 'change').pipe(
    map(() => yearSelect.value)
  );

  // Combine initial value with changes and handle data updates
  yearChange$.pipe(
    startWith(yearSelect.value),
    map(year => salesData[year])
  ).subscribe({
    next: updateCharts as any,
    error: (err) => {
