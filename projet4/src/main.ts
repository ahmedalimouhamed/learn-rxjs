...
class StockSimulator{
    private stocks$ = new BehaviorSubject<StockData[]>(initialStocks);
    private alerts$ = new BehaviorSubject<Alert[]>([]);

    createStockStream(symbol: string, initialPrice: number, initialVolume: number){
        return interval(CONFIG.updateInterval).pipe(
            scan((acc: StockData) => {
    ...
    simulateMarket(){
        const streams = initialStocks.map(stock => 
            this.createStockStream(stock.symbol, stock.price, stock.volume)
        );

        return combineLatest(streams).pipe(
            map(stocks => stocks.sort((a, b) => a.symbol.localeCompare(b.symbol))),
            tap(stocks => this.stocks$.next(stocks)),
            tap(stocks => this.checkAlerts(stocks)) 
        )
    }

    private checkAlerts(stocks: StockData[]){
    ...
    addRandomStock(){
        ...
        const symbol = <string> newSymbols[Math.floor(Math.random() * newSymbols.length)];
...
class FinancialDashboard{
    private simulator = new StockSimulator();
    private charts: any = {};
    ...
    private setupEventListeners(){
        ...
        fromEvent(document.getElementById('speedSelect')!, 'change').subscribe((event: any) => {
    ...
    private updateStockDisplay(stocks: StockData[]){
    ...
    private updateCharts(stocks: StockData[]){
        ...
        if(this.charts.price.data.labels.length > CONFIG.maxDataPoints){
            ...
            this.charts.price.data.datasets.forEach((dataset: any) => {
                dataset.data.shift();
            })
        }
    ...
    private updatePortfolio(stocks: StockData[]){
