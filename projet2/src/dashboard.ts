...
class MonitoringDashboard{
    ...
    simulateMetric(metricName: keyof typeof this.metrics){
        return interval(1000 + Math.random() * 2000).pipe(
            map(() => {
    ...
    createeAlerts(){
        return interval(5000).pipe(
            map(() => {
                const alerts = [];
                if(this.metrics.cpu > 85) alerts.push('CPU usage high!');
                if(this.metrics.memory > 90) alerts.push('Memory usage critical');
                if(this.metrics.network > 80) alerts.push('Network traffic high');
                return alerts;
            }),
    ...
    createDashboard(){
        ...
        return merge(cpu$, memory$, network$, disk$).pipe(
            scan((acc, metric) => ({
                ...acc,
                [metric.metric]: metric.value
            }), {cpu: 0, memory: 0, network: 0, disk: 0}),
            startWith({cpu: 0, memory: 0, network: 0, disk: 0})
        )
    }
}
