...
class DataService{
    ...
    processData(data: any[]): Observable<any>{
        return of(data).pipe(
            map(items => items.filter(item => item.value > 50)),
            tap(items => console.log('Items filtrés : ', items.length)),
            switchMap(items => this.transformItems(items))
        )
    }

    private transformItems(items: any[]): Observable<any>{
...
class DataProcessor{
    static calculateStats(...values: number[]): {avg: number; max: number; min: number}{
...
of('users', 'products', 'orders').pipe(
    tap(endpoint => console.log('Traitement endpoint : ', endpoint)),
    switchMap(endpoint => {
