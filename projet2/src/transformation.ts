...
).subscribe(user => console.log('filter actifs > 25 : ', user));

from(numbers).pipe(
    reduce((acc, val) => acc + val, 0)
).subscribe(result => console.log('reduce total : ', result));

from(numbers).pipe(
    scan((acc, val) => acc + val, 0)
).subscribe(accumulation => console.log('Scan accumulation : ', accumulation));

from(users).pipe(
    map(user => user.name)
).subscribe(name => console.log('noms : ', name));

from(numbers).pipe(
    distinctUntilChanged()
).subscribe(unique => console.log('distinctUntilChanged : ', unique));

from(users).pipe(
    take(2),
    skip(1)
).subscribe(user => console.log('take(2) + skip(1) : ', user))