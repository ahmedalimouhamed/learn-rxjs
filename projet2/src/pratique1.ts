...
function createCachedObservable(){
    ...
    return of("Data from expensive operation").pipe(
        delay(1000),
        tap(() => console.log('Appel API effectué : ', ++callCount)),
        shareReplay(1)
    )
}
...
const userState$ = new BehaviorSubject<{name: string; loggedIn: boolean}>({
    name: 'John Doe',
    loggedIn: false
});
...
searchTerm$.pipe(
    debounceTime(300),
    switchMap(term => 
        of(`Resultat pour ${term}`).pipe(delay(500))
    )
).subscribe(results => 
    console.log('Resultats de recherche : ', results)
);
...
function withLoading<T>(observable: Observable<T>): Observable<T>{
