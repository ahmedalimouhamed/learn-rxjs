...
class ChatSystem {
    ...
    currentUser = 'vous';
    simulateOtherUsers() {
        return interval(3000 + Math.random() * 7000).pipe(map(() => {
    ...
    getChatStream() {
        ...
        return merge(this.messages$, otherUsers$).pipe(scan((messages, newMessage) => [...messages, newMessage].slice(-10), []));
    }
    createTypingIndicator() {
        return this.messages$.pipe(filter(msg => msg.user === this.currentUser), switchMap(() => of('typing...').pipe(switchMap(() => timer(2000).pipe(map(() => ''))))), startWith(''));
    }
}
...
//# sourceMappingURL=chat-websocket.js.map