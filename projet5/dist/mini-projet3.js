...
orderStream$.subscribe(order => console.log(`Nouvelle commande : ${order.client} a acheté ${order.product} (${order.price}€)`));
orderStream$
    .pipe(map(order => order.price), scan((acc, curr) => acc + curr, 0))
    .subscribe(total => console.log(`chiffre d'affaires total : ${total}€`));
orderStream$
    .pipe(filter(order => order.client === 'Alice'))
    .subscribe(order => console.log(`commande spéciale : Alice a acheté ${order.product}`));
orderStream$
    .pipe(bufferCount(3))
    .subscribe(batch => console.log(`Analuse batch de 3 commandes : `, batch.map(o => o.product)));
const promoStream$ = interval(5000).pipe(take(3), map(i => `promo spécial -${10 * (i + 1)}% sur tout !`));
merge(orderStream$, promoStream$).subscribe(event => console.log("Evenement : ", event));
setTimeout(() => {
...
//# sourceMappingURL=mini-projet3.js.map