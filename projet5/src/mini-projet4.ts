...
const orderStream = new Subject<{client: string; dish: string}>();

const preparedOrders$ = orderStream.pipe(
    filter(order => order.dish.startsWith("p")),
    map(order => ({
        ...order,
...
const combined$ = combineLatest([preparedOrders$, timer$]).pipe(
    map(([order, time]) => ({
        ...order,
...
const finishedOrders$ = preparedOrders$.pipe(
    map(order => ({
        ...order,
        status: "finished"
    }))
